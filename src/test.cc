...
// extern "C" double custom_log2(double a) {
//   return a + 2;
// }
...
int main() {
  ...
  FunctionType *FuncTy = FunctionType::get(Type::getDoubleTy(Context),
                                           {Type::getDoubleTy(Context)}, false);

  // 创建log函数
  Function *LogFunc =
      Function::Create(FuncTy, Function::ExternalLinkage, "custom_log2", M);
  ...
  // 创建执行引擎
  std::string ErrorStr;
  ExecutionEngine *EE = EngineBuilder(std::unique_ptr<Module>(M))
